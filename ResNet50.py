...
import torch
import distance_minimization as disMin
...
class MFUDA(nn.Module):

    def __init__(self, num_classes=4):
        super(MFUDA, self).__init__()
    ...
    def forward(self, data_src, data_tgt = 0, label_src = 0, mark = 1):
        ...
        if self.training == True:
            ...
            if mark == 1:
                ...
                #mmd_loss += mmd1.mmd(data_src, data_tgt_son1)
                #mmd_loss += coral_loss(data_src, data_tgt_son1)
                #mmd_loss += wasserstein_distance(data_src, data_tgt_son1)
                mkmmd = disMin.MKMMDLoss()
                mmd_loss += mkmmd(data_src, data_tgt_son1)
                ...
                pred_tgt = torch.nn.functional.softmax(pred_tgt_son1, dim=1)
                loss_csa = csa_loss(pred_src, pred_tgt_son1, (label_src == pred_tgt.data.max(1)[1]).float())

                cls_loss = F.nll_loss(F.log_softmax(pred_src, dim=1), label_src)

                return cls_loss, mmd_loss, l1_loss / 2, loss_csa
            ...
            if mark == 2:
                ...
                data_src = self.avgpool(data_src)
                data_src = data_src.view(data_src.size(0), -1)
                mmd_loss += mmd1.mmd(data_src, data_tgt_son2)
                #mmd_loss += coral_loss(data_src, data_tgt_son2)
                #mmd_loss += wasserstein_distance(data_src, data_tgt_son2)
                mkmmd = disMin.MKMMDLoss()
                mmd_loss += mkmmd(data_src, data_tgt_son2)
                ...
                pred_tgt = torch.nn.functional.softmax(pred_tgt_son2, dim=1)
                loss_csa = csa_loss(pred_src, pred_tgt_son2, (label_src == pred_tgt.data.max(1)[1]).float())
            ...
            if mark == 3:
                ...
                data_src = self.avgpool(data_src)
                data_src = data_src.view(data_src.size(0), -1)
                mmd_loss += mmd1.mmd(data_src, data_tgt_son3)
                #mmd_loss += coral_loss(data_src, data_tgt_son3)
                #mmd_loss += wasserstein_distance(data_src, data_tgt_son3)
                mkmmd = disMin.MKMMDLoss()
                mmd_loss += mkmmd(data_src, data_tgt_son3)
                ...
                pred_tgt = torch.nn.functional.softmax(pred_tgt_son3, dim=1)
                loss_csa = csa_loss(pred_src, pred_tgt_son3, (label_src == pred_tgt.data.max(1)[1]).float())
